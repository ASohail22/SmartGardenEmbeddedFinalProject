...
I2C_Mode I2C_Master_ReadReg(uint8_t dev_addr, uint8_t reg_addr, uint8_t count)
{
    ...
    MasterMode = SWITCH_TO_RX_MODE; //TX_REG_ADDRESS_MODE;
...
/**
 * Sets the pinouts for the LEDs
 */
void configureLED()
{
    //sets the green and Red LED
    P1OUT &= ~BIT2 & ~BIT1;
    P1DIR |= BIT2 | BIT1;
    //sets the yellow LED
    P5OUT &= ~BIT0;
...
/**
 * toggles the LEDs on or off based off of temperature value
 */
void testLED()
{
    if (IntDegF >= 50 && IntDegF <= 90) //light green led P1.1
    {
        P1OUT |= BIT1;
        __no_operation();
        P5OUT &= ~BIT0;
